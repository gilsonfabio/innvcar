...
export default function SearchCLientes() {
    ...
    useEffect(() => {     
      api.get(`clientes`).then(resp => {                    
        ...
        }).catch(err => {console.log('tot. clientes', clientes.length);
    
      });
    }, []); 

    useEffect(() => {
        api.get(`buscliente/${nomCliente}`).then(resp => {                    
            setClientes(resp.data);
          
            }).catch(err => {placaAlert});
    }, [nomCliente]); 

    function loadClientes(nomCliente) {
         console.log(nomCliente);
    }

    function navigateCars(cliente) {
      navigation.navigate('Carros',{cliente}); 
 }
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Image 
                    ...
                    style={styles.image}
                />
                <TouchableOpacity style={{alignSelf: 'center', justifyContent: 'center', marginTop: 20}} onPress={navigateService} >
            ...
            <View style={styles.containerSearch}>                     
                <TextInput 
                  style={styles.input } 
                  placeholder= "Informe nome Cliente" 
                  autoCorrect= {false} 
                  value={nomCliente}
                  onChangeText={setNomCliente}
                />
            ...
            <View style={styles.card}>
                <FlatList 
                    data={clientes}
                    keyExtractor = {(cliente, index) => index.toString()} 
                    renderItem={({item: cliente}) => (
                    <TouchableOpacity onPress={() => navigateCars(cliente)} >    
